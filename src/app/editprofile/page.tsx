...
import { ChevronLeft, Pencil, Coins, CircleDollarSign, Sparkles, Star, Camera } from 'lucide-react';
...
const NeoFolderInput: React.FC<NeoFolderInputProps> = ({ label, value, type = "text", bgColor = "bg-white" }) => {
    return (
        // Wrapper utama dengan animasi hover (naik ke atas dan shadow memendek)
        <div className="relative pt-8 mb-6 drop-shadow-[8px_8px_0_rgba(0,0,0,1)] group hover:drop-shadow-[2px_2px_0_rgba(0,0,0,1)] hover:translate-x-1 hover:translate-y-1 transition-all duration-300 w-full z-10">
            ...
            <div className={`absolute top-0 left-0 px-5 py-2 border-4 border-black border-b-0 rounded-t-2xl font-black text-sm sm:text-base uppercase ${bgColor} z-20 h-10 flex items-center`}>
                {label}
                {/* Garis Ajaib: Menutupi border atas dari kotak input agar terlihat menyatu */}
                <div className={`absolute -bottom-[4px] left-[2px] right-[2px] h-[6px] ${bgColor}`}></div>
            </div>

            {/* Kotak Input */}
            <div className="relative z-10">
                {/* Perhatikan: rounded-tr-2xl membuat sudut kanan atas melengkung, sudut kiri atas siku-siku (menyatu dengan tab) */}
                <input
                    type={type}
                    defaultValue={value}
                    className={`w-full ${bgColor} text-black border-4 border-black rounded-b-2xl rounded-tr-2xl px-5 py-4 sm:py-5 font-black text-lg sm:text-xl focus:outline-none`}
                />
                {/* Tombol Edit */}
                <button className="absolute right-4 top-1/2 -translate-y-1/2 bg-white border-4 border-black p-2 rounded-xl hover:bg-black hover:text-white transition-all active:scale-90">
...
export default function App() {
    ...
    return (
        // Background utama: Oranye persik dengan pola polkadot halus
        <div className="min-h-screen bg-[#F4A261] font-sans flex items-center justify-center p-4 sm:p-8 relative overflow-hidden"
            ...
            {/* Ornamen Latar Belakang agar "Hidup" */}
            ...
            <div className="bg-[#81D4FA] w-full max-w-5xl border-4 border-black drop-shadow-[16px_16px_0_rgba(0,0,0,1)] rounded-3xl p-6 sm:p-10 relative z-10 flex flex-col">
                ...
                <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 items-stretch">
                    ...
                    {/* KOLOM KIRI: Avatar & Keluar */}
                    <div className="flex flex-col items-center lg:items-stretch gap-6 w-full lg:w-56 shrink-0 z-20">
                        {/* Box Avatar */}
                        <div className="bg-white border-4 border-black rounded-3xl w-48 h-48 lg:w-full lg:h-auto lg:aspect-square flex items-center justify-center p-4 drop-shadow-[8px_8px_0_rgba(0,0,0,1)] relative group cursor-pointer hover:-translate-y-2 transition-transform duration-300">
                            {/* Ilustrasi Avatar Gadis (Dibuat semirip mungkin dengan referensi) */}

                            {/* Overlay Edit saat di-hover */}
                            <div className="absolute inset-0 bg-black/60 rounded-[1.3rem] opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity">
                                <Pencil className="text-white" size={40} strokeWidth={2.5} />
                            </div>
                        </div>
                    ...
                    {/* KOLOM TENGAH: Form Input (Tab Folders) */}
                    <div className="flex-1 flex flex-col justify-start">
                        <NeoFolderInput label="Nama Kamu" value="Iswara" bgColor="bg-white" />
                        <NeoFolderInput label="Email Kamu" value="Iswara @gmail.com" bgColor="bg-[#E0FBFC]" />
                        <NeoFolderInput label="Ultah Kamu" value="02-05-2005" bgColor="bg-[#FFC8DD]" />
                    </div>

                    {/* KOLOM KANAN: Widget Top Up */}
                    <div className="w-full lg:w-[320px] shrink-0 relative mt-8 lg:mt-0 lg:ml-4 z-20">

                        {/* Kartu Top Up (Sedikit Miring/Rotated) */}
                        <div className="bg-[#F4D03F] w-full border-4 border-black rounded-3xl p-8 flex flex-col items-center justify-center drop-shadow-[12px_12px_0_rgba(0,0,0,1)] transform lg:rotate-3 hover:rotate-0 transition-transform duration-300 h-full">
                            ...
                            {/* Badge "20 Coins" Melayang di ujung */}
                            ...
                            {/* Ilustrasi Tumpukan Koin (Tidak Bouncing) */}
